...
static void ConvertTypeToDiagnosticString(
    ...
    ArrayRef<intptr_t> QualTypeVals, bool ShowColor) {
  auto Color = ShowColor ? StringRef(&ToggleHighlight, 1) : "";
  ...
  if (!Repeated) {
    ...
    if (const auto *VTy = Ty->getAs<VectorType>()) {
      ...
      auto ElemTy = VTy->getElementType().getAsString(Context.getPrintingPolicy());
      OS << "'" << Color << S << Color;
      OS << "' (vector of " << VTy->getNumElements() << " '";
      OS << Color << ElemTy <<  Color;
      OS << "' " << Values << ")";
      return;
    }
  }
  ...
  switch (Kind) {
    ...
    case DiagnosticsEngine::ak_declcontext: {
      ...
      if (DC->isTranslationUnit()) {
        ...
        ConvertTypeToDiagnosticString(OS, Context,
                                      Context.getTypeDeclType(Type), PrevArgs,
                                      QualTypeVals, ShowColors);
      } else {
