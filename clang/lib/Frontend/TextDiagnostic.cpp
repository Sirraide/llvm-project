...
void TextDiagnostic::emitFilename(StringRef Filename, const SourceManager &SM) {
  ...
  // If the fancy format is enabled, try to simplify paths that contain
  // '..' in any case since paths to standard library headers especially
  // tend to get quite long otherwise. Only do that for local filesystems
  // though to avoid slowing down compilation too much.
  auto SimplifyInFancyFormat = [&] {
    return DiagOpts.getFormat() == TextDiagnosticFormat::Fancy &&
           File->getName().contains("..") &&
           llvm::sys::fs::is_local(File->getName());
  };

  if (File && (DiagOpts.AbsolutePath || SimplifyInFancyFormat())) {
