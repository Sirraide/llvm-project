...
llvm::FunctionCallee CodeGenModule::GetOrCreateCXXContractViolationHandler() {
  if (!CXXContractViolationHandler) {
    ...
    if (TargetABI.isItaniumFamily()) {
      ...
      name = "?handle_contract_violation@@YAXAEBVcontract_violation@contracts@"
             "std@@@Z";
    } else {
    ...
    auto *Ptr = llvm::PointerType::getUnqual(VMContext);
    auto *HandlerTy = llvm::FunctionType::get(VoidTy, Ptr, /*isVarArg=*/false);
    auto *Handler = cast<llvm::Function>(GetOrCreateLLVMFunction(
        name, HandlerTy, GlobalDecl(), /*ForVTable=*/false,
        /*DontDefer=*/false, /*IsThunk=*/false));

    CXXContractViolationHandler = {HandlerTy, Handler};
  }

  return CXXContractViolationHandler;
}
